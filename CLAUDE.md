...
| `send_kindle_{id}` | Подготовить файл для Kindle |
...
- Kindle отправка реализована как "скачай и перешли": бот скачивает файл и отправляет пользователю с инструкцией переслать на kindle email. SMTP не используется.
- Приоритет форматов для Kindle: `["epub", "mobi", "pdf", "fb2"]` (`config.KINDLE_FORMATS`).

#### Пользовательские настройки

- `books_per_page` — 5, 10 или 20 (дефолт 10).
- `default_format` — формат скачивания по умолчанию (дефолт `fb2`).
- `kindle_email` — email для Kindle.
...
1. **Все функции БД — синхронные.** Они вызываются из async-хэндлеров напрямую (блокируют event loop). При росте нагрузки нужно мигрировать на aiosqlite или `run_in_executor`.
2. **`_book_from_cache` — тоже синхронная.** Может дополнительно делать HTTP-запрос к Flibusta (через `flib.get_book_by_id`), блокируя event loop.
