...
async def help_command(update: Update, context: CallbackContext):
    """/help — same menu as /start but with a different greeting."""
    ...
    if ALLOWED_USERS and user_id == ALLOWED_USERS[0]:
        if ALLOWED_USERS:
            ...
            for uid in ALLOWED_USERS:
                ...
                if user_data:
                    users_info.append(f"• {uid} — {escape_md(user_data.get('full_name', 'Неизвестно'))}")
                else:
...
async def button(update: Update, context: CallbackContext) -> None:
    ...
    # ── Branches with custom callback answer ──
    ...
    if data.startswith("full_ann_"):
        ...
        if book and book.annotation:
            ...
            ann_text = _screen(
                "📝 *Аннотация*",
                f"📖 _{escape_md(book.title)}_\n\n{escape_md(book.annotation)}",
    ...
    # ── Default answer for navigation branches ──
    await query.answer()
...
async def inline_query(update: Update, context: CallbackContext) -> None:
    """Inline mode: quick title search."""
    if ALLOWED_USERS:
