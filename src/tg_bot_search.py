...
from src.tg_bot_presentation import escape_md
...
async def search_by_author(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        context.user_data["search_type"] = "автору"
...
async def find_the_book(update: Update, context: CallbackContext) -> None:
    ...
    # ── Check if we're awaiting favorite-search input ──
    ...
    if awaiting == "fav_search":
        ...
        text = f"🔍 *Поиск в избранном: «{escape_md(search_string)}»*\n\nНайдено: {len(results)}\n"
        kb = []
