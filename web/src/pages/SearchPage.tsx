  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-2">
        <div
          className="flex items-center gap-2.5 rounded-[14px] px-3.5 py-3 transition-shadow duration-200"
          style={{
            backgroundColor: 'var(--tg-theme-secondary-bg-color, #f0f0f0)',
            boxShadow: isFocused ? '0 0 0 2px var(--tg-theme-button-color, #2481cc)' : 'none',
          }}
        >
        ...
        {(['title', 'author'] as SearchType[]).map(type => {
          ...
          return (
            <button
              ...
              className="px-3.5 py-1.5 rounded-full text-[13px] font-semibold transition-all duration-200 relative"
              style={{
            ...
            >
              ...
              {!isActive && (
                <div className="absolute inset-0 rounded-full" style={{ backgroundColor: 'var(--tg-theme-secondary-bg-color, #f0f0f0)' }} />
              )}
              ...
              {totalPages > 1 && (
                <div className="flex justify-center items-center gap-4 py-5">
                  <motion.button whileTap={{ scale: 0.95 }} onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1}
                    className="px-4 py-2 rounded-full text-[13px] font-semibold disabled:opacity-25" style={{ backgroundColor: 'var(--tg-theme-secondary-bg-color)', color: 'var(--tg-theme-text-color)' }}>
                    Назад
                  </motion.button>
                  <span className="text-[13px] font-medium" style={{ color: 'var(--tg-theme-hint-color)' }}>{page} / {totalPages}</span>
